...
async def generate_readme(data: ReadmeRequest):
    ...
    # Optional: Add safety settings if you notice 500 errors on certain repos
    safety_settings = [
...
"""
    ...
    try:
        # FIX: Pass the safety settings here
        response = model.generate_content(prompt, safety_settings=safety_settings)
        
        # Check if response has parts/text before accessing it
        if not response.text:
             raise HTTPException(status_code=500, detail="Gemini API returned an empty result.")
        
        generated_readme = response.text.strip()
        
        # Cleanup markdown fences
        if generated_readme.startswith("```markdown"):
            generated_readme = generated_readme[11:-3].strip()
        ...
        print(f"Server Error: {str(e)}") # This will show up in your terminal
